...
1. `facts/auth/invite.py` — `needs()` requires the signer be **`admin`**:
   ```python
   def needs(f):
       pk = f.body.get("pk", "")
...
- **One file per family** under `facts/auth/`. `__init__.py` stays a pure
...
| 3.1 | `facts/auth/user_invite.py` (was `invite.py`) | `needs`: `admin`→`member`; docstring states "any member or founder may invite". |
...
| 3.5 | `core/keychain.py` *(new)* | node-level plural identities above workspaces (mirror `local_signer_secret`). |
| 3.6 | `facts/auth/device.py`, `device_invite.py` *(new, optional)* | the user↔device split + the direct-key grant form (§6); enables user-grouping vs device-grouping in the measurement. |
| 3.7 | `facts/auth/admin.py` *(new, optional)* | admin-grant fact (member-gated by founder root) if evict authority should delegate. |
...
- `core/keychain.py`: `add_identity()`, `identity(kid) -> (sk, pk)`, `default()`,
