...
def build_orders(fids, fact_of):
    ...
    rank_ts = {
        fid: rank
        for rank, fid in enumerate(
            sorted(fids, key=lambda item: (timestamps[item], item)))
    }
    rank_author = {
        fid: rank
        for rank, fid in enumerate(sorted(
            fids,
            key=lambda item: (
                author_of(facts[item]), timestamps[item], item),
        ))
    }
    rank_deleg = {
        fid: rank
...
def measure(order, fids, fact_of, deps_of, rank, topology, targets=None):
    ...
    def annotate(node):
        leaves_under[id(node)] = info[id(node)][0]
        if not node["leaf"]:
            annotate(node["L"])
            annotate(node["R"])
