...
def test_star_reproduces_the_auth_core_and_ordering_regression(tmp_path):
    ...
    delegation = results["deleg"]

    assert timestamp["half"] == auth_facts
    assert delegation["half"] < author["half"] < auth_facts
    assert delegation["rho"] < author["rho"] < timestamp["rho"]
    # Guard the measured advantage, not just the ranking: a delegation
    # regression near timestamp scatter would still satisfy the checks above.
    assert delegation["mean_tax"] < timestamp["mean_tax"] / 4
    assert delegation["mean_tax"] < author["mean_tax"] / 2
    assert delegation["crossover"] < timestamp["crossover"]


def test_user_order_beats_device_author_order_with_plural_devices(tmp_path):
    _, _, stats, topology, results = run_benchmark(
        tmp_path, "random", scale=2047, devices_per_user=3)
