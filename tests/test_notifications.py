...
def _hint(node, workspace, *fids):
    ...
    return publication_hint(workspace, ApplyResult(
        "applied", node.reader(workspace).root_bytes, tuple(fids)))
...
def _hosted_event(tmp_path):
    source, workspace, secret, push_node, endpoint = _world(
        tmp_path, "source")
    ...
    assert run(applier.apply(run(applier.stage(
        "bootstrap", baseline)))).status == "applied"
    event = message.post(
        source, workspace, "general", "one pile", ts=4)
    raw = closed_subset(source, workspace, (event,))
    return source, workspace, secret, push_node, endpoint, event, raw, store


def test_exact_one_pile_result_becomes_a_post_success_hint(tmp_path):
    (_source, workspace, _secret, _push_node, endpoint, event,
     raw, store) = _hosted_event(tmp_path)
    applier = RepositoryApplier(workspace, store)
    result = run(applier.apply(run(applier.stage("member", raw))))
    hint = publication_hint(workspace, result)

    assert result.status == "applied"
    assert hint is not None and hint.root == result.root
    intent, = derive(
        hint, lambda oid: store.get("obj/" + oid))
    assert (intent.event, intent.endpoint) == (event, endpoint)


def test_cas_loser_cannot_emit_a_notification_hint(tmp_path):
    (_source, workspace, _secret, _push_node, _endpoint, _event,
     raw, store) = _hosted_event(tmp_path)
    first = RepositoryApplier(workspace, store)
    second = RepositoryApplier(workspace, store)
    source = run(first.stage("member", raw))
    ...
    assert publication_hint(workspace, won) is not None
    assert lost.status == "stale"
    assert publication_hint(workspace, lost) is None
