...
from deploy.repository_apply_wire import APPLY_RESULT_SCHEMA
...
class RecordingApplier:
    ...
    async def __call__(self, key, digest):
        ...
        return {"schema": APPLY_RESULT_SCHEMA, "status": self.status}
...
def world(tmp_path, *, apply=None, signer=None, clock=None, lease=None):
    ...
    return node, workspace, public, proof, clock, signer, apply, broker
...
def test_open_returns_the_only_put_and_finalize_invokes_exact_applier(tmp_path):
    ...
    expected = ingress_key(
        workspace, result.session, member, leaf.digest)
...
@pytest.mark.parametrize(
    "raw_status", ("applied", "noop", "rejected", "retryable"))
def test_finalize_has_one_small_provider_neutral_result(
    ...
    *_, proof, _clock, _signer, _apply, broker = world(
    ...
    assert asyncio.run(broker.finalize(lease.cursor)) \
        == FinalizedUpload(raw_status)


@pytest.mark.parametrize(
    "alias", ("admitted", "confirmed", "rejected-staging", "missing",
              "rootless", "stale"))
def test_finalize_rejects_internal_status_aliases(tmp_path, alias):
    *_, proof, _clock, _signer, _apply, broker = world(
        tmp_path, apply=RecordingApplier(alias))
    lease = opened(broker, proof)
    with pytest.raises(UploadUnavailable, match="private Applier"):
        asyncio.run(broker.finalize(lease.cursor))
