...
from full_peer.upload_journal import UploadJournalError, UploadSourceBuilder
...
def source(tmp_path, pile=b'{"facts":[],"ws":"' + b"a" * 64 + b'"}'):
    return UploadSourceBuilder(
        tmp_path / "uploads", WORKSPACE, MEMBER).finish(pile)
...
def test_sender_puts_one_exact_immutable_pile_then_pokes_applier(tmp_path):
