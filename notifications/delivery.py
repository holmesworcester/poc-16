"""Derive mobile pushes from one exact published repository root.

SNS, Pub/Sub, and cron records are advisory carriers for ``PublicationHint``.
This module owns no queue or repository mutation: a
duplicate hint simply derives the same event/endpoint delivery ID again.
"""
...
PUBLISHED_STATUSES = frozenset((
    "applied", "confirmed", "noop", "already-applied"))
...
def publication_hint(workspace, result):
    """Translate only a successful ordinary root result into advisory work."""
    if getattr(result, "status", None) not in PUBLISHED_STATUSES \
            or not isinstance(getattr(result, "root", None), bytes):
        return None
    admitted = tuple(sorted(set(getattr(result, "admitted", ()))))
    return None if not admitted else PublicationHint(
        workspace, result.root, admitted)
